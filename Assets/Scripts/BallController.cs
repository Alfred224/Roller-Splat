...
public class BallController : MonoBehaviour
{
    ...
    // Explosion
    public ParticleSystem explosionSound;
    ...
    private void Start()
    {
        ...
        // Assigning audio variable and particle color variable
        audioManager = GameObject.Find("Main Camera").GetComponent<AudioSource>();
        explosionRenderer = GameObject.Find("Crash_Particle").GetComponent<ParticleSystemRenderer>();
        explosionRenderer.material.color = solveColor;
    }
    ...
    private void FixedUpdate()
    {
        ...
        // Play move sound
        if (rb.velocity == Vector3.zero && playCount == 0)
        {
            audioManager.PlayOneShot(hitSound);
            explosionSound.Play();
            playCount = 1;
        } else if (rb.velocity != Vector3.zero && playCount >= 0)
        {
            playCount = 0;
        }
